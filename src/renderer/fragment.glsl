...
void main() {
  ...
  vec2 constituentPosition = vConstituent.xy + vec2(ivec2(mod(vConstituentOffset, vec2(vConstituent.zw))));
  ...
  } else /*if (vComposition == COMPOSITION_CONSTITUENT_MASK)*/ {
    gl_FragColor = vec4(sourcePx.rgb, constituentPx.a);
  }
}
