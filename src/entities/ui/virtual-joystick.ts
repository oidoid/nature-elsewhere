...
export class VirtualJoystick {
  ...
  update(milliseconds: number, recorder: Recorder): void {
    ...
    if (joystick) {
      this.setStick(joystick.normal, joystick.magnitude)
      this._timer = 0
    } else this.setStick({x: 0, y: 0}, 0)
  }

  images(): ReadonlyArray<Image> {
    return this.hidden() ? [] : this._images.images()
  }

  private setPosition(position: XY): void {
    this._timer = 0
    this._images.moveTo(XY.add(position, -this._radiusPx))
  }

  private setStick(normal: XY, magnitude: number): void {
