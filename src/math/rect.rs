...
// - Review lerp, ceil and general project stuff.
// - Should I rename ceil to ceil_mul?
// - How do I feel about `Anim` instead of `Animation`?
// - Am I using `Option` / `Result` instead of panic where I want to? Maybe
//   allow the root program to fail.
...
// - Try to split out aseprite-atlas again. This should still generate typings.
//   Also publish as crate. Note the last pure TypeScript release which will
//   probably be much lighter weight in terms of bytes. How to handle math
//   dependencies now that duck-typing can't be used? Consume the crate, not the
//   NPM package, from nature elsewhere.
